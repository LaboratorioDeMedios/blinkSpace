...
- all the pixels from the vga frasme and its size
- the coordinates of the face
- the points and vertices, and animation units
- if jittered is true then the same random values are added to every x and y every y.
...
void FaceFrame::init (float fullFrameWidth, float fullFrameHeight,
	...
	FT_VECTOR2D * facePoints, UINT facePointCount, FT_VECTOR2D * faceVertices, UINT vertexCount, 
	FLOAT * animUnits, UINT animUnitCount, bool jittered) {
		expectancy = ofRandom(MAX_AGE / 2, MAX_AGE);
		age = 0;
		...
		if (jittered) {
			int jitX = ofRandom (JITTER);
			int jitY = ofRandom (JITTER);
			translate (jitX, jitY);
		}
...
void FaceFrame::setFaceImage(unsigned char * fullFramePixels) {
		...
		faceRegion.translate(-faceRegion.x + 35,-faceRegion.y + 55); // todo fix this!!
...
void FaceFrame::updateScreenPos() {
	x = faceVertices[0].x; y = faceVertices[0].y;
}

...
void Traces::keyPressed(int key){
	...
	case 'e':
		eyeAnalizer.referenceEyesAvailable = false;
		break;
...
void Traces::updateStatus() {
	...
	status << "getting vertices (g): " << faceTrackerManager->gettingVertices << 
		", showing vertices (x): " << drawSuperImposedVertices << ", showing traces (c): " << showTraces << ", traces size: " << facesManagerTraces.getNumberOfFaces() << endl << endl;
...
void Traces::draw(){
	...
	if (showTraces){
		facesManagerTraces.render(tracesAlpha);		 
